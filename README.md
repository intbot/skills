...
| **sharpen** | Rewrites a rough, unclear prompt into a sharp one for an AI agent — states the goal up front, splits multiple asks into ordered tasks, names implied context — then **confirms the final with you** (✅ use · ✏️ tighten · ➕ add context · 💬 chat) before printing a paste-ready block. Slash-only: `/sharpen <your rough prompt>`. |
